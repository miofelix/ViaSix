...
应用不安装系统扩展或网络扩展。应用包包含一个由 `SMAppService` 管理的 LaunchDaemon helper；管理员必须在系统设置批准服务后它才有资格运行。主应用和现有 Xray/CFST 仍以当前用户权限运行，helper 当前不执行网络修改或用户目录中的二进制。
...
10. 从 `/Applications` 启动，注册 helper 后正确区分“未注册”“等待系统设置批准”“已启用”和“不可用”；测试完成后正常注销服务。
