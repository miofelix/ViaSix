...
- 真正的 TUN 实现需要一个经过 Developer ID 签名的特权 helper（优先评估 `SMAppService`），由 helper 负责创建/关闭受控 `utun`、应用/恢复路由和 DNS，并通过窄白名单 IPC 接收已校验的请求。
- helper 只能使用应用签名目录中的 Xray 和配置，拒绝任意路径、任意命令和未签名替换；安装、签名要求、用户授权和卸载都必须有独立状态。
...
启用顺序：检查能力 → 恢复/保存旧状态 → 关闭系统代理 → 启动 helper 与 Xray → 等待接口、路由和进程就绪 → 发布已启用状态。

停止顺序：停止接收新连接 → 恢复 DNS 和路由（使用 CAS，保留外部修改）→ 停止 Xray → 关闭 TUN/helper → 删除会话快照。崩溃或强制退出时，下次启动先完成同一恢复流程；恢复失败时不能假装网络已经恢复。
