...
- [x] 全量路由 + 用户态 IPv4/IPv6 TCP + 通用 UDP（SOCKS5 CONNECT / UDP ASSOCIATE；`Tun2SocksEngine`）
- [x] 用户态 TCP 转发加固（会话上限 / 写队列 / 重传去重）
- [x] UDP NAT 表 + framing 单元测试；DNS/53 ASSOCIATE 失败回退
- [x] 无系统代理（产品矩阵已约定）
...
- [x] 五分区工作流对齐 macOS（首页 / 优选 / 配置 / 日志 / 设置）
- [ ] 可选：native hev/tun2socks（替换用户态转发；非生产阻断项）
