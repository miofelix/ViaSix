...
- 自动模式默认使用 `https://api.myip.la/cn?json` 检测出口 IP，用户也可以指定其他 HTTP / HTTPS 服务；强制 IPv4 / IPv6 时分别使用 `https://api-ipv4.ip.sb/ip` 与 `https://api-ipv6.ip.sb/ip`。每次主检测成功后，ViaSix 还会把检测到的 IP 放入 `https://api.ip.sb/geoip/<IP>` 请求，以获取国家、地区、城市、网络运营商、ASN 和时区；该请求沿用当前的直连或本地代理路径。地理信息请求失败不会影响 IP 结果。
